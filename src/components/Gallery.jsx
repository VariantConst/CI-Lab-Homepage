              ...
              {images.map((image, index) => (
                <div key={image.id} className="w-full flex-shrink-0">
                  <img
                    src={image.src + "?w=960&fit=fill"}
                    alt={`Image ${image.id}`}
                    className="w-full object-cover transition duration-700 ease-in-out transform hover:scale-105 hover:cursor-pointer"
                    onClick={() => {
                      setSelectedImageId(image.id);
                      setIsModalOpen(true);
                    }}
            ...
            <div className="absolute bottom-1 sm:bottom-2 lg:bottom-4 left-1/2 transform -translate-x-1/2 flex items-center space-x-1 sm:space-x-2">
              {images.map((_, index) => (
                <div
                  ...
                  } transition duration-300 ease-in-out transform hover:scale-125 cursor-pointer`}
